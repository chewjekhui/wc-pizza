...
/**
 * @todo : impostare option per slot temporali da utilizzare per sudivisione orari, su admin, se si cambia, vengono cancellati gli orari
 */

if(in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {

	if (!class_exists( 'overplace_103722' ) ) {
		 ...
		 *
		 */
		class overplace_103722{
			
			/**
			 * Text domaine
			 * @var string
			 */
			private static $plugin_text_domain = 'overplace_103722';
			...
			public function __construct(){
				...
				add_action('pa_ingredienti-aggiuntivi_add_form_fields', array($this,'wppizza_ingredienti_fields'), 10, 2);
				add_action('pa_ingredienti-aggiuntivi_edit_form_fields', array($this,'wppizza_ingredienti_fields'), 10, 2);
				add_action('edited_pa_ingredienti-aggiuntivi', array($this,'save_ingredienti_prezzo_callback'), 10, 2);
				add_action('created_pa_ingredienti-aggiuntivi',array($this,'save_ingredienti_prezzo_callback'), 10, 2);
				...
				add_filter( 'woocommerce_in_cart_product_title', array($this, 'wppizza_cart_title'), 10,2 );
			...
			public function wppizza_order_status_observer($id_order){
				...
				$orario = $order->order_custom_fields['wppizza_orario'][0];
			...
			public function wppizza_orario_field_update_order_meta($order_id){
				if ($_POST['wppizza_orario']) update_post_meta( $order_id, 'wppizza_orario', esc_attr($_POST['wppizza_orario']));
			}
			...
			public function wppizza_tpl_custom_pizza(){
				...
				$iextra = apply_filters('recupera_ingredienti_extra', array_values(woocommerce_get_product_terms($product->id, 'pa_ingredienti-aggiuntivi', 'all')));
			...
			public function render_wppizza_boxes($ibase, $iextra){
			...
			public function install_var_wppizza(){
				...
				$wpdb->query("
					...
					");
				$wpdb->query("
						INSERT INTO ".$wpdb->prefix."woocommerce_attribute_taxonomies (attribute_name, attribute_label, attribute_type, attribute_orderby) VALUES ('ingredienti-aggiuntivi', 'Ingredienti Extra', 'select', 'menu_order')
					");
			}
			...
			public function uninstall_var_wppizza(){
				...
				$wpdb->query("
						DELETE FROM ".$wpdb->prefix."woocommerce_attribute_taxonomies WHERE attribute_name = 'ingredienti-base'
					");
				$wpdb->query("
						DELETE FROM ".$wpdb->prefix."woocommerce_attribute_taxonomies WHERE attribute_name = 'ingredienti-aggiuntivi'
					");
			}
